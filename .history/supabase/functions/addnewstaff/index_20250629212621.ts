...
serve(async (req: Request) => {
  ...
  try {
    ...
    const PROJECT_URL = (Deno.env.get("PROJECT_URL") || "❌ Missing PROJECT_URL").trim();
    const SERVICE_ROLE_KEY = (Deno.env.get("SERVICE_ROLE_KEY") || "❌ Missing SERVICE_ROLE_KEY").trim();
    const FUNCTION_SECRET = (Deno.env.get("FUNCTION_SECRET") || "❌ Missing FUNCTION_SECRET").trim();
    const JWT_SECRET = (Deno.env.get("JWT_SECRET") || "❌ Missing JWT_SECRET").trim();

    logs.push(`🔑 Env PROJECT_URL: ${PROJECT_URL}`);
    logs.push(`🔑 Env SERVICE_ROLE_KEY: ${SERVICE_ROLE_KEY.slice(0, 8)}...✅ Loaded`);
    logs.push(`🔑 Env FUNCTION_SECRET: ✅ Loaded`);
    logs.push(`🔑 Env JWT_SECRET: ${JWT_SECRET.slice(0, 8)}...✅ Loaded`);

    // ✅ Check Function Secret (custom header)
    const functionSecretHeader = req.headers.get("x-function-secret") ?? "";
    logs.push(`🔐 Incoming Function Secret Header: ${functionSecretHeader}`);

    if (functionSecretHeader !== FUNCTION_SECRET) {
      logs.push(`⛔ Invalid FUNCTION_SECRET. Expected: ${FUNCTION_SECRET}`);
      return new Response(
        JSON.stringify({ error: "Unauthorized - Invalid FUNCTION_SECRET", logs }),
