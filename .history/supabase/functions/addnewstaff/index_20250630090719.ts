...
serve(async (req) => {
  ...
  try {
    ...
    // ✅ Check Function Secret — support both x-function-secret and Authorization
    const authHeader = req.headers.get("authorization")?.replace("Bearer ", "").trim();
    const functionSecretHeader = req.headers.get("x-function-secret")?.trim();

    logs.push(`🔐 Incoming Auth Header: ${authHeader}`);
    logs.push(`🔐 Incoming x-function-secret Header: ${functionSecretHeader}`);

    const incomingSecret = authHeader || functionSecretHeader;

    if (incomingSecret !== FUNCTION_SECRET) {
      logs.push(`⛔ Invalid FUNCTION_SECRET. Expected: ${FUNCTION_SECRET}, got: ${incomingSecret}`);
      return new Response(
