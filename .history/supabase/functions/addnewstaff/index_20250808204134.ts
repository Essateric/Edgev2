...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      logs.push(`✅ JWT verified for ${payload.sub}`);
    } catch (err) {
    ...
    // ✅ Look up permission from `staff` table
...
if (staffErr) {
  logs.push(`⛔ Staff lookup error: ${staffErr.message}`);
...
if (!staffRows || staffRows.length === 0) {
  logs.push(`⛔ No staff record found for ${payload.sub}`);
...
const staffRow = staffRows[0];
    if (staffErr || !staffRow) {
      logs.push(`⛔ No staff record found for ${payload.sub}`);
      return new Response(
        JSON.stringify({ error: "Forbidden - No staff record found", logs }),
        { status: 403, headers: corsHeaders },
      );
    }

    if (staffRow.permission?.toLowerCase() !== "admin") {
