...
logs.push(`🔑 Env PROJECT_URL: ${PROJECT_URL}`);
logs.push(`🔑 Env SERVICE_ROLE_KEY: ${SERVICE_ROLE_KEY.slice(0, 8)}...✅ Loaded`);
logs.push(`🔑 Env FUNCTION_SECRET: ${FUNCTION_SECRET} ✅ Loaded`);
logs.push(`🔑 Env JWT_SECRET: ${JWT_SECRET.slice(0, 8)}...✅ Loaded`);

const authHeader = req.headers.get("Authorization") ?? "";
logs.push(`🔐 Incoming Auth Header Raw: ${authHeader}`);

// ✅ Log function secret check comparison
logs.push(`🔍 Comparing FUNCTION_SECRET (${FUNCTION_SECRET}) with Incoming Auth (${authHeader})`);
