...
serve(async (req) => {
  ...
  try {
    ...
    // ✅ Check x-function-secret header
    const incomingSecret = req.headers.get("x-function-secret") ?? "";
    logs.push(`🔐 Incoming x-function-secret Header: ${incomingSecret}`);
    logs.push(`🔑 Expected FUNCTION_SECRET: ${FUNCTION_SECRET}`);
