...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ✅ Validate Function Secret from header
    ...
    if (functionSecretHeader !== FUNCTION_SECRET) {
      logs.push(
        `⛔ Invalid FUNCTION_SECRET. Expected: ${FUNCTION_SECRET}, got: ${functionSecretHeader}`
      );
      return new Response(
    ...
    // ✅ Generate JWT (Optional if needed for frontend auth)
