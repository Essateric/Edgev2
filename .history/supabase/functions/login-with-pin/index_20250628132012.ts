...
serve(async (req) => {
  ...
  try {
    ...
    logs.push(`🧠 Emails in auth.users: ${authUsers.map((u) => u.email).join(', ')}`);
    ...
    logs.push(
      `🔐 Creating JWT for: ${matchedStaff.name} (${matchedStaff.email}), Role: ${permission}`
    );
    ...
    return new Response(
      JSON.stringify({ token: jwt, user: { id: authUser.id, email: authUser.email, permission }, logs }),
      { status: 200 }
    );
  } catch (err) {
