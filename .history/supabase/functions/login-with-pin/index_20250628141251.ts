...
// Helper to create a CryptoKey for signing JWT
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ✅ Fetch auth users using Supabase Auth Admin API
    ...
    logs.push(`📥 Found ${userList.users.length} auth users`);
    logs.push(
      `🧠 Emails in auth users: ${userList.users
        .map((u) => u.email)
        .join(", ")}`
    );
