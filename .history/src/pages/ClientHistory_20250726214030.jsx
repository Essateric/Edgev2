...
export default function ClientHistoryFullScreen({ clientId, isOpen, onClose, stylistList = [] }) {
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      const { data, error } = await supabase
        .from("bookings")
        .select("*") // Make sure resource_id and status are included
        ...
        {bookings.length === 0 ? (
          ...
          <table className="w-full border text-sm text-left">
            ...
            <tbody>
              {bookings.map((b) => {
                const stylistName =
                  stylistList.find((s) => s.id === b.resource_id)?.title || "N/A";
                ...
                return (
                  <tr key={b.id} className="border-b">
                    ...
                    <td className="px-3 py-2">{stylistName}</td>
