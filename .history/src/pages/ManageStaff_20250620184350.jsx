...
export default function ManageStaff() {
  ...
  ...(doc.weekly_hours || {}),
  ...Object.fromEntries(
    Object.entries(doc.weekly_hours || {}).map(([day, config]) => [
      day,
      {
        start: config?.start ?? "",
        end: config?.end ?? "",
  ...
  async function handleSubmit(e) {
    ...
    const hasInvalidServices = form.services.some(
      (s) =>
        !s.price ||
        !s.duration ||
        (s.duration.hours === 0 && s.duration.minutes === 0)
    );
...
// Replace your handleEdit with this:
function handleEdit(member) {
  const mergedWeeklyHours = Object.fromEntries(
    Object.entries(defaultWeeklyHours).map(([day, defaults]) => {
      const source = member.weekly_hours?.[day] || {};
      return [
        day,
        {
          start: source?.start ?? "",
          end: source?.end ?? "",
          off: source?.off ?? false,
        },
      ];
    })
  );
...
// Replace your weekly hours input block inside JSX with this:
