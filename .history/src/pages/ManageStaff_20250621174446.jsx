...
export default function ManageStaff() {
  ...
  console.log("STAFF_SERVICE UPSERT PAYLOAD", dataToUpsert);

  const saveStaffServices = async () => {
    for (let s of staffServices) {
      if (s.price || s.duration) {
        // Upsert (insert or update)
        await supabase
          .from("staff_services")
          .upsert([{ ...s, staff_id: editServicesStaff.id }], { onConflict: ['staff_id', 'service_id'] });
      }
    }
  ...
  .from("staff_services")
  .upsert([dataToUpsert], { onConflict: ['staff_id', 'service_id'] });
if (error) {
