// ManageStaff.jsx - With 'Edit Services' toggle added
...
export default function ManageStaff() {
  ...
  async function saveModalHours() {
    ...
    if (error) console.error("Failed to save hours:", error);
    else {
  ...
  return (
    <div className="p-6">
      ...
      <div className="mt-6">
        <button onClick={() => setShowServiceEditor(!showServiceEditor)} className="text-bronze underline">
          {showServiceEditor ? "Hide Services" : "Edit Services"}
        </button>
                  ...
                  <tbody>
                    {servicesList.filter(s => s.category === category).map(service => (
                      <tr key={service.id} className="border-t">
                        <td><input type="checkbox" className="ml-2 text-bronze border-bronze" /></td>
                        <td className="px-2 text-bronze">{service.name}</td>
                        <td><input type="number" className="w-16 border p-1 text-bronze border-bronze" /></td>
                        <td><input type="number" min={0} max={10} className="w-16 border p-1 text-bronze border-bronze" /></td>
                        <td><input type="number" min={0} max={59} className="w-16 border p-1 text-bronze border-bronze" /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {Object.entries(modalHours).map(([day, config]) => (
              <div key={day} className="flex items-center mb-2 gap-2">
                ...
                <button
                  ...
                  className={`ml-2 px-2 py-1 text-sm rounded ${config.off ? "bg-red-200 text-red-700" : "bg-gray-200 text-gray-700"}`}
                >
