...
export default function ManageStaff() {
  ...
  async function fetchData() {
    ...
    setStaff(
      staffData.map(doc => ({
        id: doc.id,
        ...doc,
        weekly_hours: doc.weekly_hours ?? defaultWeeklyHours,
      }))
    );
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const hasInvalidServices = form.services.some(s => !s.price || !s.duration || (s.duration.hours === 0 && s.duration.minutes === 0));
  ...
  function handleEdit(member) {
    setForm({
      ...
      weeklyHours: member.weekly_hours || defaultWeeklyHours,
    });
