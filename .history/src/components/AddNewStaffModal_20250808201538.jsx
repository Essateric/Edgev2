...
export default function AddNewStaffModal({ open, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        "https://vmtcofezozrblfxudauk.supabase.co/functions/v1/addnewstaff",
        {
          ...
          headers: {
            ...
            Authorization: `Bearer ${currentUser.token}`, // admin’s Supabase session
          },
      ...
      // Server returns: { user: { id, email, name, permission }, token_hash?, email_otp? }
      const { user, token_hash, email_otp } = data;

      // Create a REAL Supabase session for the new user (optional – logs this device in as them)
      if (token_hash || email_otp) {
        let out;
        if (token_hash) {
          out = await supabase.auth.verifyOtp({
            type: "magiclink",
            token_hash, // do NOT pass email with magiclink flow
          });
        } else {
          out = await supabase.auth.verifyOtp({
            type: "email",
        ...
        // Stash richer UI info so AuthContext can hydrate on next load
      ...
      onSaved?.();   // refresh staff list
      onClose?.();   // close modal
      // Your AuthContext's onAuthStateChange will pick up the new session automatically.
    } catch (err) {
      console.error(err);
