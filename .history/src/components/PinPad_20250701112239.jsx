...
export default function PinPad({ value = '', onChange, onEnter, disabled = false, loading = false }) {
  ...
  const handleDelete = () => {
    if (!isDisabled) onChange(value.slice(0, -1));
  };
  ...
  const ActionButton = ({ children, onClick, variant = 'secondary' }) => {
    ...
    return (
      <button
        ...
        `}
      >
        <span className="relative z-10 flex items-center justify-center text-5xl font-bold bg-gradient-to-b from-gray-100 via-gray-300 to-gray-600 bg-clip-text text-transparent drop-shadow-sm">
          {children}
        </span>
      </button>
    );
    ...
    <div className="w-full max-w-[420px]">
      ...
      <div className="mt-3">
        <ActionButton onClick={handleDelete}>
          <Delete size={18} className="mr-2 text-white" /> DELETE
        </ActionButton>
      </div>
    </div>
  );
