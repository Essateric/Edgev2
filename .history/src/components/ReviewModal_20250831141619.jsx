...
export default function ReviewModal({
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (userErr) {
        console.warn("Auth getUser warning:", userErr.message);
      }
      ...
      if (user?.id) {
        const { data: staffMatch, error: staffErr } = await supabase
          .from("staff")
          .select("id")
          .eq("id", user.id) // ✅ staff.id matches Auth UID
          .single();

        if (staffErr) {
          console.warn("Staff lookup failed:", staffErr.message);
        }

        logged_by = staffMatch?.id ?? user.id; // fallback to auth UUID
      } else {
        logged_by = null;
      }
      ...
      for (const service of basket) {
        ...
        currentStart = isChemical(service)
