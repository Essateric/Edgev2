...
const fallbackServices = [
  { id: 1, name: "Dry Cut", category: "Cut and Finish", base_price: 20, base_duration: 30, stylist: ["Martin", "Darren"] },
  { id: 2, name: "Wet Cut", category: "Gents", base_price: 15, base_duration: 20, stylist: ["Annalise"] },
  { id: 3, name: "Half Head Highlights", category: "Highlights", base_price: 50, base_duration: 60, stylist: ["Daisy"] },
];
...
export default function NewBooking({
  ...
  // Fetch staff-specific service overrides for this stylist
  useEffect(() => {
    let isMounted = true;
    async function fetchOverrides() {
      if (!stylistId) { setStaffServiceOverrides([]); return; }
    ...
    fetchOverrides();
    // re-fetch when stylistId changes
  }, [stylistId]);
  ...
  // Given a service, return the "display" price/duration for this stylist
  function getPriceAndDuration(service) {
    // Try staff override first
    const staffOverride = staffServiceOverrides.find(
      ss => ss.service_id === service.id
    );
    // Use staff-specific price/duration if present, else base from service
    return {
      price: staffOverride?.price != null ? staffOverride.price : service.base_price,
      duration: staffOverride?.duration != null ? staffOverride.duration : service.base_duration,
    };
  }

  // Add a service to the basket with correct price/duration
  ...
  // Totals
  const totalCost = basket.reduce((sum, s) => sum + (Number(s.displayPrice) || 0), 0);
  const totalDuration = basket.reduce((sum, s) => sum + (Number(s.displayDuration) || 0), 0);
        ...
        <div className="overflow-y-auto col-span-1">
          {filteredServices.length === 0 ? (
            ...
            filteredServices.map((service, i) => {
              const { price, duration } = getPriceAndDuration(service);
              const mins = duration ? Number(duration) % 60 : 0;
              const hrs = duration ? Math.floor(Number(duration) / 60) : 0;
              return (
                <div
                  key={i}
          ...
          {basket.length === 0 ? (
            ...
            <ul className="space-y-3">
              {basket.map((item, index) => {
                const mins = item.displayDuration ? Number(item.displayDuration) % 60 : 0;
                const hrs = item.displayDuration ? Math.floor(Number(item.displayDuration) / 60) : 0;
                return (
                  <li
                    key={index}
          ...
          {basket.length > 0 && (
            <div className="mt-4">
              <p className="font-semibold text-sm text-bronze">
                Total: £{totalCost.toFixed(2)} • {Math.floor(totalDuration / 60)}h{" "}
                {totalDuration % 60}m
              </p>
