...
export default function NewBooking({
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error) {
          ...
          setClients(data || []);
          // Debugging log: See what you get!
  ...
  // Helper: Bulletproof client display
  const formatClientLabel = (client) => {
    ...
    const last =
      typeof client.last_name === "string" && client.last_name.trim()
        ...
        : "";
    const mobile =
      typeof client.mobile === "string" && client.mobile.trim()
        ? client.mobile.trim()
        : "No number";
  ...
  const renderClientSelect = () => (
    <div className="mb-4">
      ...
      {clientsLoading ? (
        ...
        <div className="text-red-600">Error: {clientsError}</div>
      ) : !clients.length ? (
...
>
  <option value="">Choose a client...</option> console.log("Clients array in select:", clients);

  {clients.map(c => (
    <option key={c.id} value={c.id}>
      {`${c.first_name || "No First Name"} ${c.last_name || ""} - ${c.mobile || "No number"}`}
    </option>
  ))}
</select>
  ...
  // Display label for selected client in booking summary
  const client = clients.find((c) => c.id === selectedClient);
  const basketClientLabel = formatClientLabel(client);

  // Debug: Log the selected client object when it changes
  useEffect(() => {
    if (selectedClient) {
