...
export default function PinPad({ value = '', onChange, onEnter, disabled = false }) {
  ...
  const PinButton = ({ children, onClick }) => (
    <button
      ...
      `}
    >
      <span className="relative z-10 text-2xl font-bold bg-gradient-to-b from-gray-100 via-gray-300 to-gray-600 bg-clip-text text-transparent drop-shadow-sm">
        {children}
      </span>
    </button>
  );
  ...
  const ActionButton = ({ children, onClick, variant = 'secondary' }) => {
    ...
    return (
      <button
        ...
        `}
      >
        <span className="relative z-10 flex items-center justify-center text-lg font-bold bg-gradient-to-b from-gray-100 via-gray-300 to-gray-600 bg-clip-text text-transparent drop-shadow-sm">
          {children}
        </span>
      </button>
    );
