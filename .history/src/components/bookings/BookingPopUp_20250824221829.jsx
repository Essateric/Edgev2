...
export default function BookingPopUp({
  ...
  // Hook to manage/save DOB (updates `clients.dob` DATE column)
  const { dobInput, setDobInput, savingDOB, dobError, saveDOB } =
    useSaveClientDOB();

  const client = clients.find((c) => c.id === booking?.client_id);

  // Debug: current user (kept from your original code)
  useEffect(() => {
    let mounted = true;
  ...
  // Seed DOB input from client (keep input as YYYY-MM-DD for the <input type="date" />)
  useEffect(() => {
    if (!client) return;
    if (client?.dob) {
