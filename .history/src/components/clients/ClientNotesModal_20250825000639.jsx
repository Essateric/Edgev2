...
export default function ClientNotesModal({ isOpen, onClose, clientId }) {
  ...
  const resolveCurrentStaffName = async () => {
    try {
      ...
      // Try to match staff by auth_id, then email
  ...
  useEffect(() => {
    ...
    // Resolve stylist name up-front too
    (async () => {
  ...
  // History (NOTE: no 'notes' column in bookings → removed from select)
  useEffect(() => {
    if (!isOpen || !clientId) return;
    let active = true;
...
const handleAddNote = async () => {
  ...
  // Resolve author right now (robust fallback chain)
  const resolveAuthor = async () => {
    const { data: { user } = {} } = await supabase.auth.getUser();
    if (!user) return "Stylist";
  ...
  console.log("🖊️ Note author resolved:", author);

  const payload = {
    client_id: clientId,
    note_content: text,
    created_by: author,                // 👈 will now be the actual stylist’s name
    booking_id: bookingId || null,     // 👈 link note to the current booking (see below)
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Client Details" className="w-full max-w-[640px]">
      <div className="space-y-4 text-gray-800">
        ...
        {/* HISTORY TABLE (Notes column shows — because bookings.notes doesn’t exist) */}
        <div>
