tart, title, category, notes, resource_id")import { useEffect, useMemo, useState } from "react";
 ...
 * - bookingId: UUID of the specific booking row this modal was opened from (optional but recommended)
 */
export default function ClientNotesModal({ isOpen, onClose, clientId, bookingId }) {
  ...
  // Resolve current stylist (Auth → staff row); returns a display string
  const resolveCurrentStaffName = async () => {
    try {
      const { data: { user } = {} } = await supabase.auth.getUser();
      if (!user) return staffName || "Stylist";

      // Try exact match on auth_id
      ...
      if (byAuth) return byAuth.name || byAuth.email || byAuth.permission || (user.email ? user.email.split("@")[0] : "Stylist");
      ...
      if (byEmail) return byEmail.name || byEmail.email || byEmail.permission || (user.email ? user.email.split("@")[0] : "Stylist");

      // Last-chance fallbacks from Auth
      return (
  ...
  // History (NOTE: bookings has no "notes" column)
  useEffect(() => {
    if (!isOpen || !clientId) return;
    let active = true;
    ...
    (async () => {
      ...
      // provider map
      const ids = Array.from(new Set(rows.map((b) => b.resource_id).filter(Boolean)));
      if (ids.length) {
      ...
      // booking metadata used when rendering note link (date + title)
      const meta = {};
      rows.forEach((b) => {
  ...
  const handleAddNote = async () => {
    ...
    // Ensure we have a reliable author at insert time
    const author =
      staffName && staffName !== "Stylist"
        ? staffName
    ...
    const payload = {
      ...
      // ✅ link to the booking this modal was opened for (if provided)
