...
if (!error) {
  ...
  if (data.mobile) setMobileInput(data.mobile);
}
  ...
  const { error } = await supabase
    .from("clients")
    .update({ email: emailInput, mobile: mobileInput })
    .eq("id", clientId);

  if (!error) {
    setIsEditingContact(false);
    setClient({ ...client, email: emailInput, mobile: mobileInput });
  } else {
  ...
  <div className="space-y-2">
    ...
    <div className="flex gap-2 text-xs mt-1">
      <button onClick={handleSaveContactInfo} className="text-green-600">Save</button>
      <button onClick={() => {
        setIsEditingContact(false);
    ...
    >
      Edit
        ...
        <div className="flex gap-2">
          ...
          />
          <Button onClick={handleAddNote}>Add Birthday</Button>
        </div>
