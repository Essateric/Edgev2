...
export default function ClientNotesModal({ isOpen, onClose, clientId, bookingId = null }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        .from("bookings")
        .select("id, start, title, category, resource_id")  // <- removed notes
        .eq("client_id", clientId)
      ...
      // Meta to print on each note (when we have booking_id)
      const meta = {};
      rows.forEach(b => {
        const when = isNaN(new Date(b.start)) ? "—" : format(new Date(b.start), "dd MMM yyyy");
        meta[b.id] = {
          when,
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Client Details" className="w-full max-w-[640px]">
      <div className="space-y-4 text-gray-800">
        ...
        {/* HISTORY TABLE (Notes column is "—" because bookings has no 'notes' field) */}
        <div>
          ...
          {history.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  {history.map((h) => {
                    const dateStr = isNaN(new Date(h.start)) ? "—" : format(new Date(h.start), "dd MMM yyyy");
                    ...
                    return (
                      <tr key={h.id} className="border-b align-top">
                        ...
                        <td className="py-2 pr-0"><span className="text-gray-500">—</span></td>
                      </tr>
                    );
                  })}
                </tbody>
