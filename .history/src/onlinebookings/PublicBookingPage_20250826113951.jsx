...
// IMPORTANT: We removed the static import of supabase to avoid path errors like
// "File not found: ../../supabaseClient.js". Instead we lazy-load it at runtime
// from common locations, with a final fallback to env-based creation.
let __sb = null;
async function getSupabase() {
  if (__sb) return __sb;
  // Try typical locations FIRST (adjust as you like):
  try {
    const m = await import("../supabaseClient.js");
    __sb = m.supabase || m.default || m;
    return __sb;
  } catch {}
  try {
    const m = await import("../../supabaseClient.js");
  ...
  // Fallback: build from env (Vite-style env variables)
  try {
    ...
    console.error("Supabase client could not be resolved.", e);
    throw new Error(
      "Supabase client not found. Place supabaseClient.js next to src (../ or ../../), or set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY."
    );
  }
}
