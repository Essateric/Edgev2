...
// ✅ Correct relative path from src/onlinebookings/PublicBookingPage.jsx → src/supabaseClient.js
import { supabase } from "../supabaseClient.js";

// PublicBookingPage.jsx — Plain JavaScript (no TypeScript)
// PUBLIC FLOW: 1) Service → 2) Stylist → 3) Time (calendar + slots) → 4) Client
...
const BUSINESS = {
  ...
  mapsEmbedSrc:
    "https://www.google.com/maps?q=The+Edge+HD+Salon+Sale&output=embed",
};
...
// Robust parser for staff.weekly_hours by weekday.
// Accepts keys: "0".."6", "sun".."sat", "sunday".."saturday" (any case).
// Accepts a single object or an array of {start,end} per day.
function getWindowsForWeekday(weekly_hours, weekday) {
  if (!weekly_hours) return [];

  let wh = weekly_hours;
  // Defensive: sometimes JSONB may come back as a string if mis-saved.
  if (typeof wh === "string") {
    try {
      wh = JSON.parse(wh);
    } catch {
  ...
  // normalise and validate HH:mm
  return arr
    .map((w) => {
      if (!w || !w.start || !w.end) return null;
      const norm = (s) => {
        const parts = String(s).split(":").map((x) => Number(x));
        const hh = isFinite(parts[0]) ? pad(parts[0]) : "00";
        const mm = isFinite(parts[1]) ? pad(parts[1]) : "00";
        return `${hh}:${mm}`;
      };
...
function buildSlotsFromWindows(date, windows, stepMins, durationMins) {
  ...
  for (const w of windows) {
    ...
    wStart.setHours(sh || 0, sm || 0, 0, 0);
    const wEnd = new Date(date);
    wEnd.setHours(eh || 0, em || 0, 0, 0);

    for (
      let t = new Date(wStart);
      addMinutes(t, durationMins) <= wEnd;
...
function runDevTests() {
  ...
  const a1 = new Date(d),
    a2 = addMinutes(a1, 60);
  const b1 = addMinutes(a1, 30),
    b2 = addMinutes(b1, 60);
  results.push({ name: "Overlap true", passed: rangesOverlap(a1, a2, b1, b2) === true });
  const c1 = new Date("2025-01-01T11:00:00"),
    c2 = new Date("2025-01-01T12:00:00");
  ...
  const whNames = { mon: { start: "09:00", end: "17:00" } };
  results.push({ name: "Weekly hours parse (named key)", passed: getWindowsForWeekday(whNames, 1).length === 1 });
  const whString = JSON.stringify({ "3": { start: "10:00", end: "14:00" } });
  results.push({ name: "Weekly hours parse (string JSON)", passed: getWindowsForWeekday(whString, 3).length === 1 });

  const slots = buildSlotsFromWindows(
  ...
  const slotsEdge = buildSlotsFromWindows(
    new Date("2025-01-02T00:00:00"),
  ...
  results.push({
    name: "Slots edge (no overflow beyond window)",
    passed:
      slotsEdge.length === 1 &&
      slotsEdge[0].getHours() === 9 &&
      slotsEdge[0].getMinutes() === 0,
  });
  const noOverlapBoundary = rangesOverlap(
...
export default function PublicBookingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // 1) Windows for weekday (robust keys + string JSON supported)
        ...
        // 2b) If selected date is today, hide times in the past
        const now = new Date();
  ...
  async function saveBooking() {
    ...
    if (
      ...
      alert(
        "Please enter your first & last name, and at least email or mobile."
      );
      return;
    }
    setSaving(true);
    try {
      ...
      const { data: ins, error: bi } = await supabase
        .from("bookings")
        .insert([payload])
        .select("*")
        .single();
      ...
      {[
        ...
        <div
          ...
          className={`pl-3 border-l-4 ${
            step === s.n ? "border-black" : "border-gray-300"
          }`}
        >
          <div
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
            ...
            <section className="bg-white rounded-2xl shadow p-4">
              ...
              {!services.length && (
                <p className="text-sm text-gray-600">
                  No services found. Add some in the DB.
                </p>
              )}
                ...
                {services.map((svc) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-gray-500 flex items-center gap-3">
                      ...
                      {svc.base_price != null && (
                        <span>{money(svc.base_price)}</span>
                      )}
                    </p>
                    {svc.category && (
                      <p className="text-xs text-gray-500 mt-1">{svc.category}</p>
                    )}
                  </button>
                ))}
              </div>
            ...
            <section className="bg-white rounded-2xl shadow p-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-sm text-black/70 hover:text-black disabled:opacity-40"
                >
                  Next →
                </button>
              </div>
                      ...
                      availableSlots.map((t) => (
                        <button
                          ...
                          className={`px-3 py-2 rounded-lg border text-sm ${
                            selectedTime &&
                            t.getTime() === selectedTime.getTime()
                              ? "border-black bg-gray-50"
              ...
              {saved && (
                <div className="mt-6 p-4 border rounded-xl">
                  ...
                  <p className="text-sm text-gray-700 mt-1">
                    Thanks, {saved.client.first_name}. Your {" "}
                    {saved.service.name.toLowerCase()} is booked with {" "}
                    {saved.provider.name}.
                  </p>
                  <div className="mt-3 grid sm:grid-cols-2 gap-3 text-sm">
                    <div className="p-3 rounded-xl border">
                      ...
                      <p className="font-medium">
                        {new Date(saved.booking.start).toLocaleString(
                          undefined,
                          {
                            weekday: "short",
                            year: "numeric",
                    ...
                    <div className="p-3 rounded-xl border">
                      ...
                      <p className="font-medium">
                        {saved.service.name} · {saved.service.base_duration} mins {" "}
                        {saved.service.base_price != null ? `· ${money(saved.service.base_price)}` : ""}
                      </p>
                    </div>
                    <div className="p-3 rounded-xl border">
                      <p className="text-gray-500">Provider</p>
                      <p className="font-medium">{saved.provider.name}</p>
                    </div>
                  </div>
                </div>
              )}
      ...
      <footer className="max-w-6xl mx-auto px-4 py-10 text-center text-xs text-gray-500">
        <p>Powered by Essateric Solutionss</p>
      </footer>
    </div>
  );
}
