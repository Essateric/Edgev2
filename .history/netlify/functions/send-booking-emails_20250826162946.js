...
function makeTransporter() {
  ...
  // Gmail transporter (use App Password if Gmail with 2FA)
...
export async function handler(event) {
  try {
    ...
    // Payload shape should match your frontend:
    // { customerEmail, businessEmail, business:{name,address,timezone}, booking:{start,end}, service:{name}, provider:{name} }
    ...
    if (!businessEmail || !isEmail(businessEmail)) {
    ...
    const tz = esc(business.timezone || "Europe/London");
    ...
    const customerSubject = `Booking confirmed – ${bizName}`;
    const customerHtml = `
      <div style="font-family:Arial,sans-serif;line-height:1.5">
        <h2 style="margin:0 0 8px 0">${customerSubject}</h2>
        <p style="margin:0 0 8px 0">Thanks for booking <b>${serviceName}</b> with <b>${providerName}</b>.</p>
        <p style="margin:0 0 4px 0"><b>When:</b> ${esc(when)} (${tz})</p>
    ...
    const customerText =
      `Booking confirmed – ${business.name || "The Edge HD Salon"}\n` +
      `Service: ${service.name || "a service"}\n` +
      `Provider: ${provider.name || "our team"}\n` +
      `When: ${when} (${business.timezone || "Europe/London"})\n` +
    ...
    if (customerEmail && isEmail(customerEmail)) {
      tasks.push(
        transporter.sendMail({
          ...
          html: customerHtml,
          text: customerText,
          replyTo: businessEmail, // so replies go to salon
        })
      );
    }
